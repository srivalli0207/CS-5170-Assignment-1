...
def check_file(file: str, check_aid: str):
    ...
    with zipfile.ZipFile(file, 'r') as zz:
        ...
        for info in zz.infolist():
            ...
            assert info.filename.startswith(target_prefix), \
                'There should only be one top-level dir (with your github id as the dir-name) inside the zip file.'
    ...
    # --
    assert target_prefix[:-1] == check_aid, f"ID mismatched: {target_prefix[:-1]} vs {check_aid}"
    print(f"Read zipfile {file}, please check that your github-id is: {target_prefix[:-1]}")
